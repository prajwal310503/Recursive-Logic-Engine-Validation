    ...
    <div className="relative animate-fade-up">
      ...
      >
        ...
        <div className="p-5 pl-6">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            {hasChildren && (
              <button onClick={handleCollapse}
                className="btn-press w-6 h-6 rounded-lg flex items-center justify-center transition-all duration-200"
            ...
            {isRoot && <span className="text-[10px] text-slate-500 uppercase tracking-widest">Root</span>}

            {hasChildren && (
              <span className="text-[10px] px-2 py-0.5 rounded-full text-slate-500"
                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}
              >{node.collapsed ? `${node.children.length} hidden` : `${node.children.length} child${node.children.length > 1 ? 'ren' : ''}`}</span>
            )}

            <div className="flex-1" />

            <button onClick={() => setShowNote(v => !v)}
              className="btn-press w-7 h-7 rounded-lg flex items-center justify-center transition-all duration-200"
              style={{
            ...
            <button onClick={handleDelete}
              className="btn-press flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg text-slate-500 hover:text-red-400 transition-all duration-200 hover:bg-red-950/30"
            ><X size={12} /> {isRoot ? 'Reset' : 'Delete'}</button>
          </div>
          ...
          {showNote && (
            <textarea value={node.note} onChange={handleNote}
              ...
              rows={2}
              className="w-full mb-3 px-3.5 py-2.5 rounded-xl text-xs text-slate-300 placeholder-slate-600 outline-none transition-all duration-200 resize-none input-glow animate-fade-in"
          ...
          {/* Cross-link badge */}
          {linkedNode && (
            <div className="flex items-center gap-2.5 mb-3 px-3.5 py-2.5 rounded-xl text-xs"
              style={{
            ...
            >
              <Share2 size={13} style={{ color: isCycle ? '#f87171' : '#fbbf24', flexShrink: 0 }} />
              ...
              <button onClick={handleUnlink} className="ml-auto text-slate-500 hover:text-white transition-colors btn-press">
                <X size={12} />
              </button>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={handleAddChild}
              className="btn-press flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-xl transition-all duration-200 hover:scale-105"
              style={{ background: accent.badge, border: `1px solid ${accent.badgeBorder}`, color: accent.text }}
            ><Plus size={12} /> Add Child</button>

            <button onClick={() => setShowLinkModal(true)}
              className="btn-press flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-xl transition-all duration-200 hover:scale-105"
              style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.25)', color: '#fcd34d' }}
            ><Share2 size={12} /> {node.linkedTo ? 'Change Link' : 'Link to…'}</button>

            {!isRoot && (
              <button onClick={handleDuplicate}
                className="btn-press flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-xl transition-all duration-200 hover:scale-105"
                style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', color: '#6ee7b7' }}
                title="Copy this node as a sibling"
        ...
        {hasChildren && !node.collapsed && (
          <div className="mx-4 mb-4 pl-5 border-l-2 border-dashed space-y-3"
        ...
        {hasChildren && node.collapsed && (
          <div className="mx-5 mb-4">
            ...
            >▸ {node.children.length} node{node.children.length > 1 ? 's' : ''} hidden — click to expand</button>
          </div>
        )}
      </div>
